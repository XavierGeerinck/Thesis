...
//    private static final String HOST = "10.0.7.11";
    ...
    public static void main(String[] args) throws Exception {
        ...
        FlinkKafkaConsumer08<CAdvisor> consumer = new FlinkKafkaConsumer08<>(
                params.getRequired("topic"), // Get topic name
        ...
        for (QueryResult.Series s : queryResult.getResults().get(0).getSeries()) {
            System.out.println(s.getTags().get("container_name"));
    ...
    public static void runPredictor(ExecutionEnvironment env, ParameterTool params, String containerName, QueryResult.Series s) throws Exception {
        ...
        Instant containerStartTime = Instant.parse("2016-12-07T10:12:25.433Z");
        ...
        if (params.has("output")) {
            ...
            System.out.println("Writing to: " + MODEL_PATH + containerName + ".txt");
            result.writeAsText(MODEL_PATH + containerName + ".txt", FileSystem.WriteMode.OVERWRITE).setParallelism(1);
            data.writeAsText(MODEL_PATH + containerName + "_normalized_data.txt", FileSystem.WriteMode.OVERWRITE).setParallelism(1);
            env.fromCollection(trainingSetList).writeAsText(MODEL_PATH + containerName + "_original_data.txt", FileSystem.WriteMode.OVERWRITE).setParallelism(1);
        }

        env.execute("Linear Regression BGD");
    }
    ...
    public static class SubUpdate extends RichMapFunction<Data,Tuple2<Params,Integer>> {
        ...
        public Tuple2<Params, Integer> map(Data in) throws Exception {
            ...
            double theta_0 = parameter.getTheta0() - LEARNING_RATE * ((parameter.getTheta0() + (parameter.getTheta1()*in.x)) - in.y);
            double theta_1 = parameter.getTheta1() - LEARNING_RATE * (((parameter.getTheta0() + (parameter.getTheta1()*in.x)) - in.y) * in.x);
